...
- 根目录结构（重点）
  ...
  - `backend/sql/*.sql`：补充或增量 DDL（生产环境可按需手动执行）。
...
- 生产环境接口地址（非常重要）：
  - 本项目前端通过 `import.meta.env.VITE_API_BASE` 读取接口基地址（默认 `http://localhost:8080/api`）。
  - 在生产创建 `frontend/.env.production` 并设置：
...
- SQL 初始化策略：
  - 首次部署可保留 `spring.sql.init.mode=always` 以自动执行 `schema.sql`。
  - 后续建议改为 `never` 避免每次重启重复初始化或意外修改结构。
  - 如果发现 `schema.sql` 执行报错或需要更细粒度控制，建议：
    - 临时设置 `spring.sql.init.mode=never`；
    - 手动执行 `backend/sql/*.sql` 中的增量脚本；
    - 或在数据库中手工创建/变更缺失表结构。
...
- 首次启动：
  - 若 `spring.sql.init.mode=always`，后端启动将自动执行 `schema.sql` 完成初始化；
  - 如遇到初始化不完整（例如收藏表缺失），请使用 `backend/sql/add_favorites_table.sql` 等增量脚本修复。
