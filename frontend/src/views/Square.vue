<!--
  拾言广场页（Square）
  职责与结构：
  - 两栏布局：左侧为 SideNav 分类导航，右侧为全宽吸顶顶栏与正文；
  - 正文由 SquareBody 渲染站点分类与卡片，并与侧栏进行滚动联动。
  数据与交互：
  - 导航数据通过 useNavigation 组合式函数加载并填充到 sections；
  - 顶栏搜索通过 onSearch(q) 与子内容联动；
  - 侧栏选择通过 bodyRef.scrollTo(id) 精确定位右侧滚动位置。
  体验与性能：
  - 顶栏默认透明，滚动到内容区时切换为纯白背景；
