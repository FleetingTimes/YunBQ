...
- 敏感变量集中管理：创建 `docker-compose.env`，包含数据库、Redis、JWT、邮件与 `SPRING_SQL_INIT_MODE`（默认 `never`），Compose 使用 `env_file` 引用。
...
- SQL 初始化策略：首次部署设 `SPRING_SQL_INIT_MODE=always` 自动执行 `schema.sql`；初始化后改为 `never`（默认）。
- 日志采样与保留：
...
- 表结构迁移：首发用 `schema.sql`（自动执行或手动执行 `backend/sql/init_schema.sql`），后续建议使用迁移工具或增量脚本。
...
- 功能验收：登录/注册、管理员页（用户、日志、导航）、消息中心、上传头像。
  - 请求链路：验证响应头含 `X-Request-Id`，并在 `request_logs/auth_logs/error_logs` 看到对应记录
