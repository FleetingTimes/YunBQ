...
```
  后端
  设置 java临时版本
    E:\Trea_WorkSpace\YunBQ\backend>set path=E:\Java\jdk-17.0.6\bin;%path%
    cd  E:\Trea_WorkSpace\YunBQ\backend>
    启动 jar包
    E:\Trea_WorkSpace\YunBQ\backend>java -jar target\backend-0.0.1-SNAPSHOT.jar
  前端
    cd E:\Trea_WorkSpace\YunBQ\frontend>
    npm run dev
```
...
> 若尚未创建数据库与表结构，请参考根目录下的 `部署.md` 中“数据库初始化”和 `backend/sql/*.sql`。
...
# 方式 1（推荐、简便）：使用轻量静态服务器（不会启用开发工具或热重载）
# 安装一次即可使用：
# npm i -g serve
# 启动静态站点：
serve -s dist -l 5173
...
> 选择“方式 1”的好处是快速，无需额外配置；选择 Nginx 则更接近生产环境。
...
> 如果你已使用 Docker，可以将 MySQL、Redis 与后端、前端静态站点统一以 Compose 启动。下方为一个最小可用示例（仅用于本地运行）。
...
> 生产化的 Docker+Nginx 配置请参考根目录的 `生产部署-Docker-Nginx.md`。
...
- 初始化表结构：参考 `backend/sql/*.sql` 与 `部署.md` 中“数据库初始化”章节。
...
若你希望我补充“一键本地运行”脚本（Windows/Linux 两套），包括自动构建、环境变量注入与启动/停止批处理，我可以在根目录新增 `scripts/` 目录并提供相应脚本文件。
